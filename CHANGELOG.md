...
- **威胁告警补充建议封堵点**：v0.3.0 实测告警能触发但模型仍"听而不从"——
  G3 白方收到"黑方活三"告警后未封堵（转攻 5,8），黑方顺势活四→连五速胜。
  现告警附带机械裁判算出的封堵坐标（如"建议落子封堵：(6,10), (6,6)"），
  提示词语气改为强制（"必须下在封堵点上…不得先走自己的进攻"）。
  实测同一活三局面：改进前白方落 5,8 失守，改进后白方立即封堵 6,6。
  威胁数据结构化（`find_threat_detail` 返回 count/grade/cells/blocks），
  `find_threats` 保持旧接口兼容。
